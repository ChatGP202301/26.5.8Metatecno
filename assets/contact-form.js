(() => {
  ...
  document.querySelectorAll("[data-contact-form]").forEach((form) => {
    ...
    form.addEventListener("submit", (event) => {
      ...
      if (Date.now() - Number(form.dataset.formStartedAt || 0) < MIN_SUBMIT_MS || hasSpamSignals(form)) {
        event.preventDefault();
        if (formStatus) formStatus.textContent = "Please check the form content and submit again.";
        return;
      }
